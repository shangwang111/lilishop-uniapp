<template>
  <view class="content">
    <u-navbar :background="navObj" :is-back="true">
    ...
    <view class="goods-content" v-if="isShowSeachGoods">
      <view class="navbar">
        ...
        <view class="nav-item" :class="{ current: filterIndex === 3 }" @click="tabClick(3, 'buy_count')">
          <text>销量</text>
          <view class="p-box">
            <view class="index-nav-arrow">
              <image class="img" src="/static/index/arrow-up-1.png"
                v-if="params.sort === 'buy_count' && params.order === 'asc'" mode="aspectFit"></image>
              <image class="img" src="/static/index/arrow-up.png" v-else mode="aspectFit"></image>
            </view>
            <view class="index-nav-arrow">
              <image class="img" src="/static/index/arrow-down.png"
                v-if="params.sort === 'buy_count' && params.order === 'desc'" mode="aspectFit"></image>
...
export default {
  ...
  onLoad(val) {
    ...
    if (this.routerVal.keyword) {
      this.params.goodsName = this.routerVal.keyword;
      this.isShowSeachGoods = true;
    }
  ...
  methods: {
    ...
    async loadData(type, loading) {
      ...
      let goodsList = await getGoodsList(this.params);
			...
			console.log(goodsList.data.result.records);
    ...
    doSearch(keyword) {
      ...
      this.isShowSeachGoods = true;
      this.$refs.mSearch.isShowSeachGoods = true;
      this.params.goodsName = this.keyword;
      this.params.pageNumber = 1;
